...
where $\varepsilon$ is the oil surface emissivity (assumed as 0.55), $\sigma$=5.67�10$^{-8}$ W/(m$^2$K$^{-4}$) is the Stefan-Boltzmann constant and $T_{air}$ is the air temperature.\
...
![Piecewise uniform representation of the flow variables.](img/constant.eps)
...
![Rectangular inlet cross section.](img/10_1.eps)
...
![Irregular inlet cross section.](img/10_2.eps)
...
![Evaluation of $d_{min}$.](img/10_3.eps)
...
![New water level for the inlet section.](img/10_4.eps)
...
![Solid wall condition.](img/10_5.eps)
