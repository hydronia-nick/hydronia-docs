...
## Oil Spill on Land for the OilFlow2D model (.OILP File)

This panel allows entering viscous fluid properties for the OilFlow2D model. To activate this panel, first select *Oil Spill on Land* on the *Components Frame* of the *Control Data* Panel.

![Oil Spill on Land Basic Model Panel (OilFlow2D model).](img/oilspillonlandpanel2.png){ width=100% }


Flow resistance relation &

1. Full Bingham
2. Simplified Bingham
3. Quadratic


- **Oil Viscosity:** Fluid viscosity (Poise or lb/in$^{2}$).
- **Oil density:** Fluid density (kg/m$^{3}$ or lb/ft$^{3}$).
- **Temperature:** Ambient temperature ($^\circ$C or $^\circ$F).
- **Yield stress:** Yield stress (Pa or lb/in$^{2}$).

![Oil Spill on Land Heat Transfer Model Panel. Environmental Parameters Tab (OilFlow2D model).](img/oilspillonlandpanel2htenv.png){ width=100% }

![Oil Spill on Land Heat Transfer Model Panel. Density Tab (OilFlow2D model).](img/oilspillonlandpanel2htden.png){ width=100% }

![Oil Spill on Land Heat Transfer Model Panel. Viscosity Tab (OilFlow2D model).](img/oilspillonlandpanel2htvis.png){ width=100% }

![Oil Spill on Land Heat Transfer Model Panel. Yield Stress Tab (OilFlow2D model).](img/oilspillonlandpanel2htys.png){ width=100% }

![Oil Spill on Land Heat Transfer Model Panel. Evaporation Tab (OilFlow2D model).](img/oilspillonlandpanel2hteva.png){ width=100% }

![Oil Spill on Land Heat Transfer Model Panel Boundary Control Tab (OilFlow2D model).](img/oilspillonlandpanel2htbc.png){ width=100% }

## Oil Spill on Water module (.OILW File)

This panel allows entering data for OilFlow2D Oil Spill on Water module. To activate this panel, first select *Oil Spill on Water* on the *Components Frame* of the *Control Data* Panel.

![Oil Spill on Water module selected in the Control Data Panel (OilFlow2D model).](img/oilspillonwaterdip.png){ width=100% }

The *Oil Spill on Water* panel includes tabs to enter data for the oil trajectory and oil weathering processes such as oil evaporation, emulsification, interaction with shorelines, dispersion, dissolution and sedimentation. Some of this processes are implemented in the present version and some will be added in forthcoming releases.

### Oil Spill on Water Control Data Tab

Use the *Control Data* panel to enter general run control parameters as summarized in Table.

![Oil Spill on Water Panel. Control Data Tab (OilFlow2D model).](img/oilspillonwaterpanel.png){ width=100% }


- **Process Switches:** Select whether to compute weathering processes.

1. Evaporation
2. Emulsification
3. Shoreline Interaction
4. Dissolution
5. Sedimentation
...
- **Simulation time (h.):** Total simulation time in hours.
- **Output Interval (h.):** Time interval for output reporting.
- **Time step (sec.):** Computational time step.
- **Wind Velocity File:** File containing wind velocity time series.
- **Ambient temperature file:** File containing temperature time series.

### Oil Spill on Water Trajectory Tab

Use the *Trajectory* panel to enter parameters associated with the oil trajectory calculation as summarized in Table.

![Oil Spill on Water Panel. Trajectory Tab (OilFlow2D model).](img/oilspillonwatertrajectorytab.png){ width=100% }


- **X (m), Y (m), Z (m):** Coordinates of the location of the spill at time = 0.
- **Initial spill time (hrs.):** Time corresponding to the start of the spill with respect to the time in the velocity field calculations.
- **N parcels:** Number of parcels or particles used to represent the oil in the particle-tracking method.
- **Spill Volume File:** File containing the time series of oil volume entering the water. The volumes are given cumulatively for each time starting from 0 and ending with the total spill volume.
- **Water density:** Density of the receiving water body.
- **Oil density:** Oil density in API units.
- **Oil viscosity:** Dynamic viscosity of the spilled oil at time = 0.
- **Dispersion coefficients:** Longitudinal, transversal, and vertical dispersion coefficients (m$^{2}$/s or ft$^{2}$/s).

### Oil Spill on Water Evaporation Tab

Use the *Evaporation* panel to enter the parameters required to compute oil evaporation as summarized in Table.

![Oil Spill on Water Panel. Evaporation Tab (OilFlow2D model).](img/oilspillonwaterevaporationtab.png){ width=100% }


- **Evaporation method:** To select either Fingas or Stiver-MacKay method.
- **Crude oil name:** Drop down list to access the Fingas method oil data base.
- **% by weight distilled at 180 $^\circ$C:** Used when selecting generic oils that follow logarithmic square root evaporation law.
- **Mass transfer coefficient:** Required in the Stiver-MackKay method.
- **Stiver-MackKay method A constant:** Constant A used in Stiver-Mackay method.
- **Stiver-MackKay method B constant:** Constant B used in Stiver-Mackay method.
