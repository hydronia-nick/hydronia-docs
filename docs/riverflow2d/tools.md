...
*See the RiverFlow2D for QGIS  Reference Manual Output Files section for more details.*
...
### Dialog Controls
#### HEEF Function Database
[]{#tab:heef_function_db_controls label="tab:heef_function_db_controls"}

| Vulnerability Function Type | *button* | Opens the Vulnerability Function Type Manager dialog. |
| --- | --- | --- |
| Vulnerability Function | *button* | Opens the Vulnerability Function Manager dialog. |

#### Vulnerability Function Type

[]{#tab:vft_manager_controls label="tab:vft_manager_controls"}

| ID | *field* | Displays the numeric identifier for the function type. |
| --- | --- | --- |
| Description | *field* | Shows the description of the function type. |
| Table Name | *field* | Displays the associated data file for this function type. |
| Previous | *button* | Navigates to the previous function type in the database. |
| Next | *button* | Navigates to the next function type in the database. |
| Add | *button* | Creates a new function type (requires appropriate permissions). |
| Del | *button* | Deletes the currently displayed function type. |
| Close | *button* | Closes the dialog and returns to the main database window. |

#### Vulnerability Function

[]{#tab:function_manager_controls label="tab:function_manager_controls"}

| Type Function | *dropdown* | Selects the category this function. |
| --- | --- | --- |
| ID | *field* | Displays the unique identifier for the selected function. |
| Description | *field* | Shows the description of the building/structure type. |
| Material | *field* | Displays the construction material. |
| Reference Photo | *field* | Shows the path to an image of the structure type. |
| Structure Cost | *field* | Shows the estimated cost of the physical structure. |
| Content Cost | *field* | Shows the estimated cost of contents within the structure. |
| Source | *dropdown* | Shows the bibliographic reference for the function data. |
| Units | *dropdown* | Shows the measurement system (metric or English). |
| Previous | *button* | Navigates to the previous function in the database. |
| Next | *button* | Navigates to the next function in the database. |
| Add | *button* | Creates a new vulnerability function. |
| Del | *button* | Deletes the currently displayed function. |
| Close | *button* | Closes the dialog and returns to the main database window. |

#### Setting Multiple Buildings
[]{#tab:structure_editor_controls label="tab:structure_editor_controls"}

| Vulnerability Function ID | *text field* | Displays the ID of the selected vulnerability function. |
| --- | --- | --- |
| Database | *button* | Opens the Vulnerability Function Selection Dialog to choose a function from the database. |
| Description | *text area* | Displays or allows editing of the description for the selected vulnerability function. |
| Building cost | *text field* | Allows entering the monetary value of the building structure. |
| Content cost | *text field* | Allows entering the monetary value of the building contents. |
| Function value (tab) | *tab* | Displays the vulnerability function values table. |
| Update Function File | *button* | Updates the function file with any changes made to the function values. |

#### Function Selection Dialog (HEEF DB)

[]{#tab:function_selection_controls label="tab:function_selection_controls"}

| Vulnerability Function Type | *dropdown* | Selects the category of function. |
| --- | --- | --- |
| Vulnerability Function | *dropdown* | Selects a specific function within the chosen category. |
| Description | *field* | Shows information about the selected function. |
| Graph Function | *button* | Displays a graphical representation of the selected function. For depth-damage functions: Shows a 2D plot of depth vs. damage percentage. For depth-velocity-damage functions: Shows a 3D surface plot of depth, velocity, and damage. |
| Reference Photo | *button* | Displays a photo of the building/structure type (if available). |
| OK | *button* | Confirms selection and returns to the Structure Editor. |
| Close | *button* | Cancels selection and returns to the Structure Editor. |

### Workflow
#### Setting Multiple Buildings
1.  Select one or more building polygons in the QGIS map canvas.

2.  Open the Structure Editor dialog.

3.  Select a vulnerability function using the Database button.

4.  Optionally adjust structure and content costs.

5.  Click OK to apply the vulnerability function to all selected buildings.

### Requirements
-   The HEEF tool requires an active RiverFlow2D project.

-   Building polygon features must be selected before opening the Structure Editor.

-   The tool supports both metric and English units for depths and velocities.

-   When the OK button is clicked in the Structure Editor, the function data is applied to all selected buildings, and categorized symbology is applied to the building layer.

### Technical Details
#### Database Structure
The vulnerability function database is organized into several text-based tables:

-   `t_tfd.txt:` Contains function type definitions.

-   `t_head_c01.txt:` Contains structure categories and materials with costs.

-   `t_fuentes.txt:` Contains bibliographic source references.

-   Individual function data files: Contain the actual damage curves.

-   `img/` folder: Contains reference photos of building types.

#### Function Data Formats
-   **Depth-Damage Functions (Type 01):** Store pairs of depth values and corresponding damage percentages.

-   **Depth-Velocity-Damage Functions (Type 02):** Store matrices of depths, velocities, and corresponding damage percentages in a 3D relationship.
